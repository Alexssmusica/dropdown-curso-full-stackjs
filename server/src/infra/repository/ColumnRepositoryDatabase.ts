...
export default class ColumnRepositoryDatabase implements ColumnRepository {
	...
	async findAllByIdBoard(idBoard: number): Promise<Column[]> {
		const columnsData = await this.connection.query("select id_board, id_column, name, has_estimative from public.column where id_board = $1", [idBoard]);
	...
	async save(column: Column): Promise<number> {
		const [columnData] = await this.connection.query("insert into public.column (id_board, name, has_estimative) values ($1, $2, $3) returning id_column", [column.idBoard, column.name, column.hasEstimative]);
		return columnData.id_column;
	}

	async get(idColumn: number): Promise<Column> {
		const [columnData] = await this.connection.query("select id_board, id_column, name, has_estimative from public.column where id_column = $1", [idColumn]);
	...
	async update(column: Column): Promise<void> {
		await this.connection.query("update public.column set name = $1 where id_column = $2", [column.name, column.idColumn]);
	}

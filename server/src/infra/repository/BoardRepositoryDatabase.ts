...
export default class BoardRepositoryDatabase implements BoardRepository {
	...
	async save(board: Board): Promise<number> {
		const [boardData] = await this.connection.query("insert into public.board (name) values ($1) returning *", [board.name]);
		return boardData.id_board;
	}

	async update(board: Board): Promise<void> {
		await this.connection.query("update public.board set name = $1 where id_board = $2", [board.name, board.idBoard]);
	}
